...
jQuery.fn.extend({
	...
	openSelect: function(options){
		return this.trigger("openSelect");
	},
	closeSelect: function(){
		return this.trigger("closeSelect");
	},
	selectValue: function(value){
		return this.trigger("selectValue", [value] );
	}
});
...
jQuery.Autocompleter = function(input, options) {
	...
	$input.keydown(function(event) {
		...
		if (!config.mouseDownOnSelect) {
			//MH: removed this cause we don't want to close combo when scrolling
			//hideResults();
		}
	}).click(function() {
		// show select when clicking in a focused field
		if ( hasFocus++ > 1 && !select.visible() && !$input.attr( 'readonly' ) ) {
			onChange(0, true);
		}
		...
		$input.focus();
	}).bind("closeSelect", function() {
		if ( select.visible() )
			select.hide();
	}).bind("selectValue", function() {
		var newVal = arguments[1];
		function findValueCallback(q, data) {
			if( data && data.length ) {
				for (var i=0; i < data.length; i++) {
					if( data[i].data.value == newVal ) {
						selectValue( data[i] );
					}
				}
			}
	...
	function openSelect( selected ){
		...
		if ( !select.visible() ) {
			...
			request('', openSelection, hideResultsNow);
		}
	...
	function selectValue( selected ) {
		...
		$input.val(v);
		$input.trigger("result", [ selected.data, selected.value, true ]);
		return true;
	}
	...
	function hideResults() {
		console.debug('hideResults()');
		clearTimeout(timeout);
		timeout = setTimeout(hideResultsNow, 200);
	};

	function hideResultsNow() {
		console.debug('hideResultsNow()');
		select.hide();
		clearTimeout(timeout);
		stopLoading();
	...
	/*
	* loads the selected from a ajax request or form the provieded data array
	*/
...
jQuery.Autocompleter.defaults = {
	inputClass: "euhComboInput",
	resultsClass: "euhComboResults",
	loadingClass: "euhComboLoading",
	minChars: 1,
	delay: 400,
	matchCase: false,
	matchSubset: true,
...
jQuery.Autocompleter.Select = function (options, input, select, config) {
	...
	function moveSelect(step) {
		...
		var listItem = listItems.slice(active, active + 1)[0];
		var top = jQuery(listItem).position().top;
		var baseDiv = jQuery(listItem).parent().parent();
		baseDiv.scrollTop( baseDiv.scrollTop() + top );
	};
	...
	function selectItem( item ){
		if(data)
		{
			...
			for (var i=0; i < num; i++) {
				...
				if( data[i] == item ){
					active = i;
					listItems.removeClass().slice(i, i+1).addClass(CLASSES.ACTIVE);
					var selected = listItems.slice(i, i+1)[0];
					var pos = jQuery(selected).position().top;
	...
	function fillList() {
		...
		for (var i=0; i < num; i++) {
			...
			if( data[i].value == '' )
				style = 'height: 14px;';
			buffer[buffer.length] = '<li name="';
			buffer[buffer.length] = i;
			buffer[buffer.length] = '">';
			buffer[buffer.length] = options.highlight(formatted, term);
	...
	return {
		...
		show: function() {
			...
			var scrollBottom = jQuery(window).scrollTop() + getPageSize()[3];
			var topPos = offset.top + input.offsetHeight;
			// MH if there is not enought space at the bottom paint the div at top of the field (like HTML selects)
			if( scrollBottom - input.offsetHeight - offset.top < element.outerHeight() )
				topPos = offset.top - element.outerHeight();
			element.css({
				width: typeof options.width == "string" || options.width > 0 ? options.width : jQuery(input).width()+14,
				top: topPos,
				left: offset.left
		...
		selected: function() {
			if( listItems && listItems.filter("." + CLASSES.ACTIVE).size() > 0 )
				return data && data[ listItems.filter("." + CLASSES.ACTIVE).attr('name') ];
		}
	};
};
